...
import be.ugent.zeus.hydra.data.database.minerva2.FullRepositoryTest;
import be.ugent.zeus.hydra.data.database.minerva2.mocks.MockCourseDao;
...
public class CourseRepositoryTest extends FullRepositoryTest<String, Course> {
    ...
    public void getIdToOrderMap() {
        ...
        for (Course course: getData()) {
            expected.put(course.getId(), course.getOrder());
        }
        Map<String, Integer> actual = courseRepository.getIdToOrderMap();
        assertEquals(expected, actual);
    }
